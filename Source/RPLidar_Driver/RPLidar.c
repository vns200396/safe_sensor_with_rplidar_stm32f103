...
{
	bool force = false;
	static uint8_t evv_detect =0;
	static rplidar_response_device_info_t info;

	stop_motor_rplidar(); //stop the rplidar motor
	...
	switch(evv_detect)
	{
		case 0:
		{
			clear_respone_device_infor_t(&info);
			evv_detect++;
			break;
		}
		case 1:
		{
			if (IS_OK(rplidar_getDeviceInfo(&info, 100)))
			{
				evv_detect++;
			}
			break;
		...
		{
			// detected...
			if (IS_OK(rplidar_startScan(force,RPLIDAR_DEFAULT_TIMEOUT*2)))
			{
				evv_detect=0;
